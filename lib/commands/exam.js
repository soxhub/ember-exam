...
module.exports = TestCommand.extend({
  ...
  _setupAndGetBrowserSocketEvents(config) {
    ...
    ui.writeLine('%%%%%%%%%%%%%%%%%% Dave was here! Setting up testem events');
    ...
    const browserExitHandler = function (failed = false) {
      ...
      if (!failed && commands.get('loadBalance')) {
        ...
        log.info('Dave was here');
        log.info(
          `Browser ${browserId} exiting. [ # of modules in current module queue ${testemEvents.stateManager.getTestModuleQueue()?.length
          } ]`
        );
